...
SOAP_SOURCE_STAMP("@(#) soapClient.c ver 2.8.35 2021-05-23 18:42:32 GMT")
...
{	struct ns__hex_and_pass soap_tmp_ns__hex_and_pass;
	...
	if (soap_endpoint == NULL)
		soap_endpoint = "http://websrv.cs.fsu.edu/~engelen/calcserver.cgi";
	soap_tmp_ns__hex_and_pass.a = a;
	soap_tmp_ns__hex_and_pass.size = size;
	soap_begin(soap);
	soap->encodingStyle = "";
	soap_serializeheader(soap);
	soap_serialize_ns__hex_and_pass(soap, &soap_tmp_ns__hex_and_pass);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__hex_and_pass(soap, &soap_tmp_ns__hex_and_pass, "ns:hex-and-pass", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	*result = NULL;
	...
	if (soap_recv_fault(soap, 1))
		return soap->error;
	soap_tmp_ns__hex_and_passResponse = soap_get_ns__hex_and_passResponse(soap, NULL, "", NULL);
	if (!soap_tmp_ns__hex_and_passResponse || soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	if (result && soap_tmp_ns__hex_and_passResponse->result)
		*result = *soap_tmp_ns__hex_and_passResponse->result;
	return soap_closesock(soap);
}
...
{	struct ns__get soap_tmp_ns__get;
	...
	if (soap_endpoint == NULL)
		soap_endpoint = "http://websrv.cs.fsu.edu/~engelen/calcserver.cgi";
	soap_tmp_ns__get.a = a;
	soap_tmp_ns__get.size = size;
	soap_begin(soap);
	soap->encodingStyle = "";
	soap_serializeheader(soap);
	...
	*result = NULL;
	...
	if (soap_recv_fault(soap, 1))
		return soap->error;
	soap_tmp_ns__getResponse = soap_get_ns__getResponse(soap, NULL, "", NULL);
	if (!soap_tmp_ns__getResponse || soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
